...
Welcome to the XDEFI Wallet Maya Protocol integration guide.

### Detect XDEFI Wallet with Maya Protocol
...
```javascript
window.xfi.mayachain.on('event_name', callback);
​//Example
window.xfi.mayachain.on('close', () => window.location.reload());
window.xfi.mayachain.on('accountsChanged', () => window.location.reload());
```
