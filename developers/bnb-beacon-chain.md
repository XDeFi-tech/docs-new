...
Welcome to the XDEFI Wallet BNB Beacon Chain integration guide.

### Detect XDEFI Wallet with BNB Beacon Chain
...
```javascript
window.xfi.binance.on('event_name', callback);
​//Example
window.xfi.binance.on('close', () => window.location.reload());
window.xfi.binance.on('accountsChanged', () => window.location.reload());
```
