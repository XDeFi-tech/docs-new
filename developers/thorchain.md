...
Welcome to the XDEFI Wallet THORChain integration guide.

### Detect XDEFI Wallet with THORChain
...
```javascript
window.xfi.thorchain.on('event_name', callback);
​//Example
window.xfi.thorchain.on('close', () => window.location.reload());
window.xfi.thorchain.on('accountsChanged', () => window.location.reload());
```
