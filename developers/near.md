...
Welcome to the XDEFI Wallet NEAR integration guide.

### Detect XDEFI Wallet with NEAR
...
```javascript
window.xfi.near.on('event_name', callback);
​//Example
window.xfi.near.on('close', () => window.location.reload());
window.xfi.near.on('accountsChanged', () => window.location.reload());
```
