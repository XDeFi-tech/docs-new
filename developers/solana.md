...
Welcome to the XDEFI Wallet Solana integration guide.

### Detect XDEFI Wallet with Solana
...
```javascript
window.xfi.solana.on('event_name', callback);
​//Example
window.xfi.solana.on('close', () => window.location.reload());
window.xfi.solana.on('accountsChanged', () => window.location.reload());
```
